...
death_owid <- rio::import(here::here("data", "owid-covid-data.csv")) %>% 
  ...
  mutate(date = as.POSIXct(date), 
    ...
    mutate(leng_iso = nchar(iso_code)) %>% 
    filter(leng_iso == 3) %>% 
...
```{r}
data_owid <- death_owid %>% rename(iso3 = iso_code, name = location) %>% select(-c(date, leng_iso, month, year)) %>% #filter(location == "United Kingdom")
  left_join(gdp_percap) %>%  
  left_join(gov_effectiveness_corruption) %>%
  left_join(democracy) %>% 
  left_join(health_index, by = c("name" = "Country")) %>% 
    left_join(trust) %>% 
  select(c(iso3, name, time_month, total_deaths, total_deaths_per_million, population,gdp_percap, democracy, gov_eff, health_index, trust_gov)) %>% 
